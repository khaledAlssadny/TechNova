...
namespace ecommerce.Controllers
{
	public class ProductController : Controller
	{
        ...
        {
            if (string.IsNullOrWhiteSpace(searchProdName))
                return RedirectToAction("GetAll");

            List<Product> searchedProducts = productService.GetAll()
                .Where(p => p.Name.Contains(searchProdName, StringComparison.OrdinalIgnoreCase))
                .ToList();

            if (!searchedProducts.Any())
            {
                TempData["SearchMessage"] = $"No products found for \"{searchProdName}\".";
                return RedirectToAction("GetAll");
            }

            // If exactly one match, go directly to its detail page
            if (searchedProducts.Count == 1)
                return RedirectToAction("Details", "Product", new { id = searchedProducts[0].Id });

            // Multiple matches: redirect to first result
            // (Phase 3 will add a proper search results page)
            int id = searchedProducts[0].Id;
        ...
        public IActionResult Update(ProductWithListOfCatesViewModel product)
		{
            ...
            // else: product.ImageUrl already holds the existing value from the hidden field
